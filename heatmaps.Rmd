...
cell_location <- sample_frac(cell_location, 0.01)
```
...
```
We are adding random constant to set as a seed for control generation.
...
Among all of the epigenetic features we are going to use CD133+:
```{r}
...
# Sites Seen Only in Single Cell Type
...
counted in only a single cell type.
